...
for root, dirs, files in os.walk(train_dir):
    for file in files:
        if file.endswith(".jpg"):
            ...
            # format is <idx>.jpg
            idx_str = file[:-4]
            idx = int(idx_str) # get as an integer, this is the corresponding index/row of camera_properties.csv
            ...
            if (not image_set_initialized):
                num_images = len(files) - 1 # assumes every image in train_data/. is an image except the camera_properties.csv files
                (w,h) = im.shape
